...
@interface NSObject (RKExtension)

@end

@implementation NSObject (RKExtension)
...
- (void)rk_arrayM_insertObject:(id)anObject atIndex:(NSUInteger)index {
    if (!anObject) {
        NSString *msg = [NSString stringWithFormat:@"%@: insert object cannot be nil at %ld", [self makeClassName:self.class AndMethod:@selector(insertObject:atIndex:)], index];
...
- (void)rk_arrayM_setObject:(id)obj atIndexedSubscript:(NSUInteger)idx {
    if (!obj) {
        NSString *msg = [NSString stringWithFormat:@"%@: set object cannot be nil at %ld", [self makeClassName:self.class AndMethod:@selector(setObject:atIndexedSubscript:)], idx];
        [self printWarningWithMesaage:msg];
        return;
    }
...
- (void)rk_arrayM_removeObjectsInRange:(NSRange)range {
    if (range.location > self.count || range.location+range.length > self.count) {
        NSString *msg = [NSString stringWithFormat:@"%@: range %@ extends beyond bounds [0 .. %ld]", [self makeClassName:self.class AndMethod:@selector(removeObjectsInRange:)] , NSStringFromRange(range), self.count-1];
        [self printWarningWithMesaage:msg];
        return;
    }
...
- (void)rk_arrayM_replaceObjectAtIndex:(NSUInteger)index withObject:(id)anObject {
    if (!anObject) {
        NSString *msg = [NSString stringWithFormat:@"%@: replace object cannot be nil at %ld", [self makeClassName:self.class AndMethod:@selector(replaceObjectAtIndex:withObject:)] , index];
        [self printWarningWithMesaage:msg];
        return;
    }
...
- (void)printBeyondWarningWithIndex:(NSUInteger)index {
    NSString *message = @"";
    if (self.count == 0) {
        message = [NSString stringWithFormat:@"index %ld beyond bounds empty array", index];
    } else {
        message = [NSString stringWithFormat:@"index %ld beyond bounds [0 .. %ld]", index, self.count-1];
    }
